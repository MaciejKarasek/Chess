...
Board::Board()
{
    for (int x = 0; x <= 7; x++)
    {
        for (int y = 0; y <= 7; y++)
        {
            board[x][y] = std::make_unique<Field>(x, y);
            boardCopy[x][y] = std::make_unique<Field>(x, y);
        }
    }
}
...
void Board::Print() {
    ...
    for (int x = 0; x < 9; x++) {
        if (x == 0) {
            ...
            std::wcout << std::format(L" {} |", 9 - x);
            for (int y = 0; y < 8; y++){
                std::wcout << std::format(L" {} |", board[x - 1][y]->StandingPiece->GetUni());
            }
